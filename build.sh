...
FIRMWARE_URL_OVERRIDES["mrvl/sd8797_uapsta.bin"]="useCustomFirmware"
...
apply_config_file() {
    ...
    while read -r action name value; do
        ...
        case "$action" in
            enable)
                scripts/config --enable "$name"
                ;;
            disable)
                scripts/config --disable "$name"
                ;;
            set-val)
...
if [[ ! -f Makefile ]] ||
   ! grep -q "KERNELRELEASE" Makefile 2>/dev/null; then
...
scripts/config \
    --set-str CONFIG_EXTRA_FIRMWARE_DIR "${FIRMWARE_DIR}"
        ...
        while IFS= read -r blob; do
            ...
            if [[ -n "${FIRMWARE_URL_OVERRIDES[$blob]:-}" ]]; then
                ...
                if [[ ${FIRMWARE_URL_OVERRIDES[$blob]} == "useCustomFirmware" ]]; then
                    ...
                    echo "Please ensure the proper firmware exists in ${FIRMWARE_DIR}/${blob}."
                    exit 1
                fi
            ...
            if [[ -n "${FIRMWARE_URL_FALLBACK}" ]]; then
                ...
                echo -e \
                    "  \e[1;33m[!] Using non-default firmware for ${blob}!\n" \
...
if [[ "$DO_BUILD" == "1" ]]; then
    ...
    echo -e "\e[1;34m[*]\e[0m Enabling built-in USB Audio..."
    ...
    # Explicitly make sure USB Audio is not a module.
    scripts/config --disable CONFIG_SND_USB_AUDIO_MODULE

    echo -e "\e[1;34m[*]\e[0m USB Audio configuration before olddefconfig:"
    grep -E '^CONFIG_(SOUND|SND|SND_USB|SND_USB_AUDIO)=' .config || true
    ...
    # ============================================================
    # Verify final USB Audio configuration
    # ============================================================

    echo -e "\e[1;34m[*]\e[0m Verifying final USB Audio configuration..."

    grep -E '^CONFIG_(SOUND|SND|SND_USB|SND_USB_AUDIO)=' .config || true

    if ! grep -q '^CONFIG_SND_USB_AUDIO=y$' .config; then

        echo -e "\e[1;31mERROR:\e[0m CONFIG_SND_USB_AUDIO is not built-in!"
        echo "Expected:"
        echo "CONFIG_SND_USB_AUDIO=y"

        exit 1
    fi

    echo -e "\e[1;32m[✓]\e[0m CONFIG_SND_USB_AUDIO=y"
